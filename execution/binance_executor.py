        ...
        # Handle FILLS
            ...
            try:
                ...
                if "Order not found" in str(e):
                    # Orphan Fill (likely from Router reprice or ghost order)
                    logger.warning(f"⚠️ Orphan fill received for {client_order_id} (ignoring safe): {e}")
                    
                    # LOG TRADES ANYWAY for Dashboard PnL accuracy
                    try:
                        orphan_fill_event = FillEvent(
                            symbol=symbol,
                            side=side,
                            quantity=last_filled_qty,
                            price=last_filled_price,
                            commission=commission,
                            pnl=0.0 # Can't calc accurate PnL without position, assume 0 for log
                        )
                        await self.db.log_trade(orphan_fill_event, strategy_id="orphan_fill")
                    except Exception as db_e:
                        logger.error(f"Failed to log orphan trade: {db_e}")
                else:
    ...
    async def _router_place_order(
        ...
        try:
            ...
            if order_type == "MARKET":
                ...
                # CRITICAL: Register order with OrderManager if not exists
                # This handles new IDs generated by the Router during repricing
                if client_order_id:
                    existing_order = await self.order_manager.get_order(client_order_id)
                    if not existing_order:
                        # Auto-create tracking record for router-generated order
                        logger.debug(f"📝 Auto-registering router order: {client_order_id}")
                        await self.order_manager.create_order(
                            ...
                            time_in_force="GTX", # Router uses GTX
                            trace_id=f"router_{client_order_id}"[-32:] # Deterministic trace
                        )
                        # Mark as we are about to submit (state sync)
                        # Actually create_order sets it to PENDING_SUBMIT, which is correct
                
                # Use GTX (post-only) for maker orders
                # If client_order_id is provided, use it (Attempt 0)
